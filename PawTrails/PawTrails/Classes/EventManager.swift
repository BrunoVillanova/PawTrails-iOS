...
class EventManager {
    ...
    private func updateSharedUsersUI(for petId: Int, in viewController: UIViewController?){
        if let petProfile = viewController?.navigationController?.viewControllers.first(where: { $0 is PetInfromationViewController }) as? PetInfromationViewController, petProfile.pet.id == petId {
