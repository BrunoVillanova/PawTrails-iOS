...
public class AirbnbDatePickerViewController: UICollectionViewController {
    ...
    var selectedStartDate: Date? {
        didSet {
            ...
            footerView.isSaveEnabled = (selectedStartDate != nil)
        }
    }
    ...
    func didSave() {
        if let del = delegate {
            if selectedStartDate != nil, selectedEndDate == nil {
                del.datePickerController(self, didSaveStartDate: selectedStartDate, endDate: selectedStartDate)
            } else if selectedStartDate == nil {

//                self.alert(title: "Error", msg: "Please select a start date", type: .green, disableTime: 3, handler: nil)
            } else {
